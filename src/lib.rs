...
impl ParsedAddress {
    pub fn new(address: &String) -> ParsedAddress {
        ...
        match Regex::new(&REGEX).unwrap().captures(&address) {
            Some(matches) => {
                parsed_address.number = matches.get(1).map_or(None, |m| Some(m.as_str().to_string()));
